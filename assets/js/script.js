...
class FluidPoint {
  ...
  update(mouse) {
    ...
    if (state.fadeStartTime) {
      const progress =
        (Date.now() - state.fadeStartTime) / CONFIG.ANIMATION.FADE_DURATION;
    ...
    if (state.enableInteraction && state.isMouseMoving && mouse.x && mouse.y) {
      ...
      if (distance < CONFIG.ANIMATION.MAX_DISTANCE) {
        const force =
          (CONFIG.ANIMATION.MAX_DISTANCE - distance) /
          CONFIG.ANIMATION.MAX_DISTANCE;
      ...
      const returnForceFactor =
        Math.min(distance / 50, 1) * CONFIG.ANIMATION.RETURN_FORCE;
  ...
  handleBoundaryCollision() {
    ...
    if (this.x < bounds.left + buffer || this.x > bounds.right - buffer) {
      ...
      this.x = Math.max(
        bounds.left + buffer,
        Math.min(this.x, bounds.right - buffer)
      );
    }
    if (this.y < bounds.top + buffer || this.y > bounds.bottom - buffer) {
      ...
      this.y = Math.max(
        bounds.top + buffer,
        Math.min(this.y, bounds.bottom - buffer)
      );
    }
  }
}
...
function setupMapPins() {
  ...
  if (!svg) {
    console.warn(
      "SVG map not found. Check if .world-map contains an SVG element."
    );
    return;
  }
  ...
  if (!viewBox || !viewBox.width || !viewBox.height) {
    console.warn(
      'SVG viewBox is not defined or invalid. Add a viewBox attribute like "0 0 1000 600".'
    );
  }
  ...
  if (!pinContainer) {
    console.warn(
      "Pin container not found. Ensure .pin-container exists in your HTML."
    );
    return;
  }
  ...
  countries.forEach((country) => {
    ...
    if (countryPath) {
      ...
      if (isMobile) {
        console.log(
          "Mobile device detected, using custom coordinates for pins."
        );
        switch (country.code) {
          case "US":
        ...
        console.log(
          "Desktop device detected, calculating pin positions based on SVG paths."
        );
        ...
        if (country.code === "US") {
          ...
          pinLeft =
            ((pathRect.left + pathRect.width / 2 - svgRect.left) /
              svgRect.width) *
              100 -
            1;
          pinTop =
            ((pathRect.top + pathRect.height / 2 - svgRect.top) /
              svgRect.height) *
              100 +
            1;
        } else {
      ...
      console.warn(
        `Country path not found for ${country.name} (${country.code}). Check SVG path IDs.`
      );
    }
  });
}
...
// Hero Carousel - Updated for 7 slides
let currentSlide = 0;
...
document
  .querySelectorAll(".product-card, .accreditation-card, .about-content > *")
  .forEach((el) => {
